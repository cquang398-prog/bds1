...
import { useContractTemplates, useDepositContracts } from '@/lib/hooks/useEntities';
...
export default function ContractsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'deposits' | 'templates'>('deposits');
  ...
  // Tab 2: Mẫu hợp đồng (templates cũ)
  const { 
    items: contractList, 
  ...
  const error = activeTab === 'deposits' ? depositsError : templatesError;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500">Quản lý hợp đồng đặt cọc giữ chỗ và mẫu hợp đồng pháp lý</p>
        </div>
      ...
      {activeTab === 'deposits' ? (
        ...
        // TABLE MẪU HỢP ĐỒNG (Templates cũ)
        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
